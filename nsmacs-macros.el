...
(defmacro e:define-ghostel-command (name &optional command)
  "COMMAND で指定したコマンドを `ghostel' で起動するコマンドを定義する.
COMMAND はシンボル/文字列、もしくは (PROGRAM ARGS...) のリストで指定する.
COMMAND が指定されていない場合は NAME をそのままコマンドとして使用する."
  (let* ((cmd (or command name))
         (tokens (if (listp cmd) cmd (list cmd)))
         (program (format "%s" (car tokens)))
         (args (mapcar (lambda (a) (format "%s" a)) (cdr tokens))))
    `(defun ,name ()
       (interactive)
       (pop-to-buffer-same-window
        (apply #'e:ghostel-exec ,(format "%s" name) ,program ',args)))))
